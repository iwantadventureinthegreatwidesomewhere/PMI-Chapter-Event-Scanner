  ...
  if (val != "") {
    ...
    guestList.map((guest) => {
      const firstNameMatch =
        guest.firstName.toLowerCase().search(searchKeyword) !== -1;

      const lastNameMatch =
        guest.lastName.toLowerCase().search(searchKeyword) !== -1;

      const fullName = `${guest.firstName} ${guest.lastName}`;
      const fullNameMatch = fullName.toLowerCase().search(searchKeyword) !== -1;

      const pmiNumberMatch =
        guest.pmiNumber.toString().toLowerCase().search(searchKeyword) !== -1;
