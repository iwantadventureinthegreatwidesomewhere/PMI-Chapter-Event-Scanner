  ...
  return (
    ...
    >
      <KeyboardAvoidingView behavior="position" enabled>
        ...
        />
        <TextInput
          onSubmitEditing={() => inputEventId.current.focus()}
          ref={inputPassword}
          blurOnSubmit={false}
          clearButtonMode="always"
          returnKeyType="next"
        ...
        >
          {login.loading ? (
            <ActivityIndicator
              style={{ color: "white", padding: 13, textAlign: "center" }}
            />
          ) : (
            <Text
              style={{
...
export function mapDispatchToProps(dispatch) {
  return {
    handleSubmit: (username, password, event_id) =>
      dispatch(loadLogIn(username, password, event_id)),
    dispatch,
  };
}
...
export default compose(withConnect, memo, withRouter)(LoginPage);
