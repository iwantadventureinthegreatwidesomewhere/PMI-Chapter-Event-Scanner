            ...
            {eventProvider.uuid != null && <SafeAreaView style={{flex:1, height: '100%', width: '100%'}}>
                <View style={{ flex:1, flexDirection:'column', height: '100%', width: '100%' }}>
                    <MainView pose={!menuAnimateState ? 'visible' : 'hidden'} style={{ height: '100%', width: '100%', zIndex: 0}}>
                        ...
                        <View style={{ alignSelf:'center', height: '100%', width:dimensions.width*0.85 , zIndex: 1 }}>
                            <TouchableOpacity onPress={() => setMenuAnimateState(true)} style={{ alignSelf: 'flex-end', marginTop: Platform.OS === 'ios' ? 25 : layout.statusBarHeight + 10 }}>
                                <Image 
                            ...
                            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'flex-end', marginBottom: 25, alignSelf: 'center', width: '100%' }}>
                                <View style={{ justifyContent:'center', padding:20, shadowColor: "#333333", shadowOffset: { width: 0, height: 9,}, shadowOpacity: 0.25, shadowRadius: 12, elevation: 19,  borderRadius: 10, marginBottom: 25, backgroundColor:'white' }}>
                                    <Text style={{ fontFamily:'Nunito-Bold', marginBottom:3, fontSize: normalizeFont(22), color:'rgb(100, 110, 227)' }}>Your event on</Text>
                                    <Text style={{ fontFamily:'Nunito-Bold', marginBottom:15, fontSize: normalizeFont(28), color:'rgb(100, 110, 227)' }}>{ t( 'date', { date: eventProvider.event.listDate }) }</Text>
                                    <Text style={{ fontFamily:'Nunito-Bold', marginBottom:10, fontSize:normalizeFont(15), color:'#333333' }}>{ Striptags(eventProvider.event.description) }</Text>
                                </View>
